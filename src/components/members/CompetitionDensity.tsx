...
import type { DensityPoint } from "@/lib/sanka-data";

type Props = { data: DensityPoint[] };
        ...
        <BarChart data={sorted} layout="vertical" margin={{ top: 0, right: 50, left: 0, bottom: 0 }}>
          ...
          <Tooltip
            ...
            formatter={(v, _, { payload }) => [
              `平均 ${v}社 / 全${payload.total_cases}案件`,
              "競合密度",
            ]}
          />
          <Bar dataKey="avg_competitors" radius={[0, 3, 3, 0]} label={{ position: "right", fontSize: 10, fill: "#6b7280" }}>
            {sorted.map((d, i) => (
              <Cell
                key={i}
                fill={
